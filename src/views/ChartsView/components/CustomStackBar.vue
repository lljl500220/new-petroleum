      ...
      tooltip: {
        ...
        formatter: function (param: any) {
          let res = `<div>${
            ...
            yData[param.dataIndex]
          }</div>`;
          res += `<div>数据二&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${
            yData2[param.dataIndex]
          }</div>`;
          return res;
        },
      },
...
const initChart = (name: string[], data: number[][]) => {
  barChart.value && echarts.dispose(barChart.value as HTMLElement);
  const chart = echarts.init(barChart.value as HTMLElement);
...
<template>
  <div ref="barChart" class="stack-bar-chart-dom" />
</template>
